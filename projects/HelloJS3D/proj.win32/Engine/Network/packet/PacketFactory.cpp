...
CPacket* PacketFactory::analyzeData(const char* data, int length)
{
	char* lastData;
	int dataSize = 0;
	...
	if(bigSize)
		...
		char *bytes = msg->getUnsignedBytes(dataSize);
	if(compress){
		unsigned char *deflated;
		unsigned int sizeHint = 20*1024;
	...
	CPacket* packet = new CPacket(lastData,dataSize);
	packet->parseHeader();
	return packet;
}
const char* PacketFactory::packPacket(CPacket* packet,int& dataLength)
{
