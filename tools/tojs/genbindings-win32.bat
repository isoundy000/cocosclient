...
echo "Generating bindings for cocos3dx..."

python %CXX_GENERATOR_ROOT%/generator.py %TO_JS_ROOT%/cocos3dx.ini -s cocos3dx -o %OUTPUT_DIR% -n jsb_cocos3dx_auto
if %errorlevel% neq 0 goto ERROR

echo "Generating bindings for engine..."

python %CXX_GENERATOR_ROOT%/generator.py %TO_JS_ROOT%/engine.ini -s engine -o %OUTPUT_DIR% -n jsb_engine_auto
if %errorlevel% neq 0 goto ERROR
